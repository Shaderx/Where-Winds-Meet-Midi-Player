...
>
  <h2 class="text-2xl font-bold mb-6">{$t("stats.title")}</h2>
  ...
  <div class="grid grid-cols-2 gap-3 mb-6">
    <div class="bg-white/5 rounded-xl p-4">
      <div class="flex items-center gap-2 mb-2">
        ...
        <span class="text-sm text-white/60">{$t("stats.songsPlayed")}</span>
      </div>
    ...
    <div class="bg-white/5 rounded-xl p-4">
      <div class="flex items-center gap-2 mb-2">
        ...
        <span class="text-sm text-white/60">{$t("stats.sessions")}</span>
      </div>
    ...
    <div class="bg-white/5 rounded-xl p-4">
      <div class="flex items-center gap-2 mb-2">
        ...
        <span class="text-sm text-white/60">{$t("stats.firstUsed")}</span>
      </div>
  ...
  <div class="bg-white/5 rounded-xl p-4 mb-6">
    <h3 class="text-lg font-semibold mb-3">{$t("nav.library")}</h3>
    <div class="grid grid-cols-3 gap-4 text-center">
      <div>
        <p class="text-2xl font-bold text-[#1db954]">{librarySize}</p>
        <p class="text-xs text-white/50">{$t("library.songs")}</p>
      </div>
      <div>
        <p class="text-2xl font-bold text-pink-400">{favoritesSize}</p>
        <p class="text-xs text-white/50">{$t("nav.favorites")}</p>
      </div>
      <div>
        <p class="text-2xl font-bold text-blue-400">{playlistsSize}</p>
        <p class="text-xs text-white/50">{$t("nav.playlists")}</p>
      </div>
    </div>
  </div>
  ...
  {#if topSongs.length > 0}
    <div class="bg-white/5 rounded-xl p-4">
      <h3 class="text-lg font-semibold mb-3">{$t("stats.mostPlayed")}</h3>
      <div class="space-y-2">
  ...
  {:else}
    <div class="bg-white/5 rounded-xl p-4 text-center text-white/40">
      ...
      <p class="text-xs">{$t("stats.startPlaying")}</p>
    </div>
  {/if}
</div>
